...
export const ExpenseReportPage: React.FC<ExpenseReportPageProps> = ({ 
  transactions, 
  financeSettings 
}) => {
  ...
  }, [expenseTransactions, searchQuery, fromDate, toDate]);

  const totalExpense = useMemo(() => {
    return filteredExpense.reduce((sum, t) => sum + t.amount, 0);
  }, [filteredExpense]);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between print:hidden">
        <div>
          ...
          <p className="text-sm text-zinc-500">Track and analyze your spending habits</p>
        </div>
        <button 
          onClick={handlePrint}
      ...
      {/* Summary Box */}
      <div className="rounded-3xl bg-rose-600 p-8 text-white shadow-xl shadow-rose-600/20 dark:bg-rose-500">
        <div className="flex items-center gap-3 opacity-80">
          <TrendingDown className="h-5 w-5" />
          <span className="text-sm font-bold uppercase tracking-wider">Total Expense for Selected Period</span>
        </div>
        <div className="mt-4 flex items-baseline gap-2">
          <span className="text-5xl font-black">{currencySymbol}{totalExpense.toLocaleString()}</span>
          <span className="text-rose-100/60 font-medium">from {filteredExpense.length} transactions</span>
        </div>
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-12 print:hidden">
        ...
        <div className="relative lg:col-span-6">
          ...
          <input
            ...
            className="w-full rounded-2xl border border-zinc-200 bg-white py-3 pl-11 pr-4 text-sm outline-none focus:ring-2 focus:ring-rose-500/20 dark:border-zinc-800 dark:bg-zinc-950 dark:text-white"
          />
        </div>
        ...
        <div className="flex items-center gap-3 lg:col-span-6">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full rounded-xl border border-zinc-200 bg-white py-2.5 pl-9 pr-3 text-xs outline-none focus:ring-2 focus:ring-rose-500/20 dark:border-zinc-800 dark:bg-zinc-950 dark:text-white"
            />
          </div>
            ...
            <tbody className="divide-y divide-zinc-200 dark:divide-zinc-800">
              {filteredExpense.map((t) => (
                <tr key={t.trn_id} className="hover:bg-zinc-50 transition-colors dark:hover:bg-zinc-900/50">
                  ...
                  <td className="px-6 py-4 font-black text-right text-rose-600 dark:text-rose-400">
